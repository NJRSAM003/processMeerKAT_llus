...
* **`archive.sh` reduction archiver** — a convenience script shipped with the pipeline (put on `$PATH` by `setup.sh`, like `processMeerKAT.py`). Run it inside a finished galaxy/target reduction directory to sweep the products worth keeping into a single `RC_PMKT_archive/` folder and then (only once you confirm) delete everything else to reclaim space. It keeps, at the top level, all `*science_image*` products — which by matching on name automatically covers the plain images, the `.alpha`/`.alpha.error` maps (`stokes != 'I'`), the `.cube.*` products and `SPW_MFSs/` (`spw_cube = True`), and the `.katbeam*` products (`pbcorr = True`) — plus `plots/`, `*.png`, and the target's own `.mms`/`.ms` (identified from `[fields] targetfields`, so calibrator MMSs are not kept). Quick-look `*.contcube[.fits]` cross-calibration diagnostics, per-SPW `*MHz` directories, calibrator MMSs and gain tables are treated as disposable. It is safe by default (preview only) and deletes only with `-y`/`--confirm-delete`; run `archive.sh --help` for options.
...
* `archive.sh` (run inside a finished reduction directory) archives the products worth keeping — `*science_image*` (images, alpha, cube, katbeam), `SPW_MFSs/`, `plots/`, `*.png`, and the target's own MMS — into `RC_PMKT_archive/`, then optionally deletes everything else. Safe by default (preview only); pass `-y`/`--confirm-delete` to delete. See `archive.sh --help`.
