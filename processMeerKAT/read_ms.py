...
def check_spw(config,msmd):
    ...
    """
    If config already gives explicit comma-separated list that is within the MS, 
    keep it exactly as written so the hard-coded window boundaries and the
    RFI gaps are preserved. If the requested range falls outside the MS, it will be clipped
    to a single min-max range.
    """
