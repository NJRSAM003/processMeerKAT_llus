...
IMAGING_OPTIONAL_KEYS = ['spw_cube','common_beam','spwid','pbcorr']
...
CONTAINER = '/software/astro/containers/casa-modular/casa-6.5.0-modular.sif'
MPI_WRAPPER = 'mpirun'
...
def write_sbatch(script,args,nodes=1,tasks=16,mem=MEM_PER_NODE_GB_LIMIT,name="job",runname='',plane=1,exclude='',mpi_wrapper=MPI_WRAPPER,container=CONTAINER,
    ...
    params = locals()
    params['LOG_DIR'] = LOG_DIR
    #Source setup.sh from this script's own directory (SCRIPT_DIR) so the generated sbatch is
    #portable rather than tied to a fixed path.
...
def write_jobs(config, scripts=[], threadsafe=[], containers=[], num_precal_scripts=0, mpi_wrapper=MPI_WRAPPER, nodes=8, ntasks_per_node=4, mem=MEM_PER_NODE_GB_LIMIT,plane=1, partition='Main',
    ...
    #Symlink archive.sh into the top-level reduction directory, alongside the other convenience
    #scripts. Skipped inside per-SPW subdirectories (names match the SPW frequency labels).
    spw_labels = set()
