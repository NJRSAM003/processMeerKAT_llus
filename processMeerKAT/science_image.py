...
# Set True when tclean is skipped (image already exists); __main__ uses this to force an MPI
# teardown at exit so the job does not idle until the SLURM time limit.
...
def make_alpha(imagename, deconvolver, stokes, alpha_nsigma=1.0):
    ...
    for label, src in [('img0', img0), ('img1', img1), ('resid0', resid0), ('resid1', resid1)]:
        ...
        if not os.path.exists(si):
            _extract_stokesI(src, si)  # ia toolkit (in-process), avoids imsubimage under MPI
        work[label] = si
...
def _extract_stokesI(imname, outname):
    ...
    Uses the toolkit rather than the imsubimage task, which under mpirun/casampi is dispatched
    through the MPI framework; the toolkit calls run in-process on the client rank instead."""
...
def _build_and_clean(vis, imagename, spw, cell, robust, imsize, wprojplanes, niter, threshold, multiscale, nterms,
    ...
    # If everything this step would produce is already on disk, skip the work and return.
    ...
    # default - it is slow on large multi-Stokes images and most users apply their own PB
    # correction. Enable it with pbcorr=True in the [image] config section.
    if not pbcorr:
...
if __name__ == '__main__':
    ...
    # If tclean was skipped, the casampi MPI servers were started at import but never used, and
    # the orphaned server ranks keep mpirun alive after the client exits. Tear down the whole MPI
    # world so the job ends promptly. Fresh parallel-tclean runs skip this.
    if _TCLEAN_SKIPPED:
        import sys, signal
        ...
        def _terminate(signum=None, frame=None):
            sys.stdout.flush()
            sys.stderr.flush()
            # os._exit(0) only ends rank 0; MPI_Abort ends every rank so mpirun returns.
            try:
